"""
* File: pricing-algorithm\webservice\generate_ratesv2.py
...
def generate_rate_old(data):
    ...
    try:
        ...
        try:
            ...
            loc_response = requests.get(f'https://maps.googleapis.com/maps/api/geocode/json?latlng={origin_lat}%2C{origin_long}&key={google_api}')
            ...
            loc_response = requests.get(f'https://maps.googleapis.com/maps/api/geocode/json?address={origincity}%2C%2B{originstate}&key={google_api}')
            loc_output = loc_response.json()
            for i in range(len(loc_output['results'][0]['address_components'])):
                if loc_output['results'][0]['address_components'][i]['types'] == ['postal_code']:
                    originzip = loc_output['results'][0]['address_components'][i]['short_name']
    ...
    try:
        ...
        try:
            ...
            loc_response = requests.get(f'https://maps.googleapis.com/maps/api/geocode/json?latlng={dest_lat}%2C{dest_long}&key={google_api}')
            loc_output = loc_response.json()
            for i in range(len(loc_output['results'][0]['address_components'])):
                if loc_output['results'][0]['address_components'][i]['types'] == ['postal_code']:
            ...
            loc_response = requests.get(f'https://maps.googleapis.com/maps/api/geocode/json?address={destcity}%2C%2B{deststate}&key={google_api}')
            loc_output = loc_response.json()
            for i in range(len(loc_output['results'][0]['address_components'])):
                if loc_output['results'][0]['address_components'][i]['types'] == ['postal_code']:
                    destzip = loc_output['results'][0]['address_components'][i]['short_name']
    ...
    try:
        ...
        des_response = requests.get(f'https://maps.googleapis.com/maps/api/geocode/json?components=postal_code:{destzip}&key={google_api}')
    ...
    #calculate GS rate
    fixed_pay = gs_fixed_prices(equipment, customer, origincity, originstate, originzip, origincountry, destcity, deststate, destzip,
                    destcountry)
    ...
    if fixed_pay is None:
        ...
        rules = get_rules(equipment, customer, origincity, originstate, originzip, origincountry, destcity, deststate, destzip,
              destcountry)
        try:
        ...
        if isinstance(dat65, int) == True and isinstance(dat35, int) == True:
            accepted_rate = min(dat65, dat35)
        elif isinstance(datrate, int) == True:
            accepted_rate = datrate
        elif isinstance(gs_rate, int) == True:
            accepted_rate = gs_rate
        else:
            accepted_rate = 'Error in rate calculation'
        type = 'nonfixed'
        if accepted_rate not in ['Error in rate calculation']:
            accepted_rate = accepted_rate + rules['combo_flat'] + rules['pri_flat']
            markup_perc = 1 + ((rules['combo_perc'] + rules['pri_perc']) / 100)
            accepted_rate = round(accepted_rate * markup_perc)
    else:
        accepted_rate = fixed_pay
        fuel_cost = 0
        type = 'fixed'
